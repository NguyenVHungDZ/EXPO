...
import type { IFaqType } from '~/interfaces/faq_type'
import useApi from '~/composables/request/useApi'
import { faqType } from '~/assets/infoData'
...
const { get, update, create } = useApi('FaqType')
const { t } = useI18n()
// get id url
...
const ruleForm = reactive<IFaqType>({
  name: '',
  description: '',
})

// validate form
const rules = reactive<FormRules<IFaqType>>({
  name: [{ required: true, message: t('input_ask'), trigger: 'blur' }],
})
...
const fetchData = async () => {
  try {
    const res: IFaqType | any = await get(id.value)
