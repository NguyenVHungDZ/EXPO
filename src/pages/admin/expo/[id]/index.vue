      ...
      <div class="pb-10">
        <div v-if="!active">
          ...
          <div class="space-x-2 flex justify-center">
            <button
              class="rounded-[10px] px-2 py-1 bg-white border text-black hover:bg-gray-100"
              @click.prevent="router.push('/admin/expo')"
            >
              {{ t('button.cancel') }}
            </button>
            <button
              class="px-2 py-1 bg-primary hover:bg-secondary rounded-[10px] text-white"
        ...
        <div v-else-if="active === 1" class="w-full pt-10 px-20">
          ...
          <div class="flex justify-center space-x-2">
            <button
              class="rounded px-2 py-1border text-black bg-[#e5e3e3] hover:bg-gray-100"
              @click="active--"
            >
              {{ t('button.previous') }}
            </button>
...
import type { IExpo } from '~/interfaces/expo'
import useApi from '~/composables/request/useApi'
import { expo } from '~/assets/infoData'
...
const { get, update, create } = useApi('Expo')
const {
  getAll,
...
// validate form
const rules = reactive<FormRules<IExpo>>({
...
const fetchData = async () => {
  try {
    const res: IExpo | any = await get(id.value)
